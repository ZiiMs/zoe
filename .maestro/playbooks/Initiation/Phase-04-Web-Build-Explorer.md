...
- [x] Add focused web tests or component-level checks where the repo already supports them:
  ...
  - Completion note: extracted pure build explorer URL helpers into `apps/web/app/builds-query.ts` and build detail route fallback helpers into `apps/web/app/builds/[id]/route-helpers.ts`, then added Vitest coverage in adjacent `*.test.ts` files for URL param parsing/defaults, filter toggle href generation, active filter/summary helper behavior, stable detail href encoding, preserved build-list context for missing-detail fallbacks, and safe route-id decoding. No React component test harness was added; this keeps the checks within the existing web Vitest setup. Validation: `bunx prettier --check apps/web/app/builds-page.tsx apps/web/app/builds-query.ts apps/web/app/builds-query.test.ts "apps/web/app/builds/[id]/page.tsx" "apps/web/app/builds/[id]/route-helpers.ts" "apps/web/app/builds/[id]/route-helpers.test.ts"`, `bun run --filter @zoe/web test` (8 tests passing), `bun run typecheck:web`, `bun run lint:web` (passes with existing `<img>` warnings), and `bun run build:web`. No images were attached or analyzed for this task.

- [x] Run web validation and fix failures:
  - `bun run typecheck:web`
  - `bun run lint:web`
  - `bun run build:web`
  - Start `bun run dev:web` and verify the build explorer loads against `NEXT_PUBLIC_API_BASE_URL` or the local API default.
  - Completion note: ran `bun run typecheck:web`, `bun run lint:web`, and `bun run build:web` successfully after adding `apps/web/app/icon.tsx` and wiring `metadata.icons` in `apps/web/app/layout.tsx` to fix the browser's missing favicon request. Lint passes with the existing `<img>` warnings only. Port 3000 was already occupied by an older Zoe dev server, so a current Next dev server was started on `http://localhost:3001`; Chrome verification confirmed the build explorer loads, renders expected controls/table content, exposes `/icon`, and has no framework overlay, bad responses, console errors, or page errors. No images were attached or analyzed for this task.
